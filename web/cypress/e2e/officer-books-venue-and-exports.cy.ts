...
/**
 * Journey 2 — the Club Officer publishes, books a room, is refused a second claim on it, and takes the
 * answers away as a file.
 *
 * The refusal is the point of the middle act. Two Events wanting the same room at the same time is the
 * other contended write in this system, and the only honest way to show it is refused is to ask for it
 * on the screen and read what the screen says back.
 */
describe("the Officer publishes, books a Venue and exports the answers", () => {
  it("refuses the second claim on one room and exports what the Students answered", () => {
    ...
    // Venues are the University Admin's records and Core gives them no screen, so this one is created
    // over the API by the only role entitled to create it.
    signIn(ADMIN);
    createVenue(venueName);

    signInThroughTheUi(OFFICER);
    publishEvent({ title: bookedTitle, description: `Wants the room. ${run}`, capacity: 5, times }).then(
      (eventId) => {
    ...
    publishEvent({
      title: clashingTitle,
      description: `Wants the same room. ${run}`,
      capacity: 5,
      times,
    }).then((eventId) => {
    ...
    cy.then(() => {
      ...
      // The room is free, and the Officer takes it.
      ...
      // The second Event asks for the same room and the same hour. The timeline shows the Slot is
      ...
      // And the refused Event still holds no room, which is what "your Event has not changed" has to
