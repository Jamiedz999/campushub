...
- [Define the code structure and what enforces it](../adr/17-define-code-structure-and-its-enforcement.md) — one public type per backend module, features that cannot import each other on the frontend, and all of it verified by Spring Modulith plus two ArchUnit rules rather than by convention. Written because "two modules never write the same document" carries the atomicity argument and was the last load-bearing rule left to discipline.
...
- **`Europe/Dublin` DST polarity corrected against the real JDK** (2026-08-13, during [#13](https://github.com/Jamiedz999/campushub/issues/13)) — [ADR 15](../adr/15-define-http-api-and-time-contract.md) assumed tzdata's *vanguard* reading (UTC+1 standard, negative winter offset). Pinning the assertion the ADR itself demanded showed OpenJDK 25 ships the *rearguard* reading instead: UTC+0 standard year-round, positive summer offset, in both `java.time` and legacy `Calendar`. Amended in place; the DST-transition Slot rule itself is unaffected.
- **Registration answers fenced to one enrollment** (2026-08-14, during [#6](https://github.com/Jamiedz999/campushub/issues/6)) — implementing the accepted second write exposed a withdraw/re-register race hidden by `(eventId, studentId)` alone. [ADRs 04](../adr/04-define-registration-capacity-and-waitlist.md) and [05](../adr/05-define-custom-registration-forms.md) now give each uninterrupted enrollment an `enrollmentVersion`, assigned by the same atomic Seat Ledger write that creates it; answer writes and reads must match it, newer writes fence out older ones, and missing legacy versions are the oldest fence.

- **A University Admin may not book an Event's Slot** (2026-08-14, during [#17](https://github.com/Jamiedz999/campushub/issues/17)) — listing the permission matrix row by row against its tests found [ADR 08](../adr/08-define-roles-and-resource-authorization.md)'s Venue Slot row contradicting [ADR 06](../adr/06-define-venue-slot-booking.md): managing a Venue and booking one are different powers, and the row collapsed them. Amended in ADR 08; the implementation followed ADR 06 all along and is unchanged. The mapping that found it is [`EVIDENCE.md`](implementation/EVIDENCE.md), which also holds the concurrency suite and the record of removing each guard to watch its test fail.
